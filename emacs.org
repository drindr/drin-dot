...
#+begin_src emacs-lisp
    (use-package rime
      ...
      ;; setup in MacOS
      ;; Map command+; into control+\ with Karabiner to align with
      ;; the system input source toggle shortcut
...
#+begin_src emacs-lisp
  ...
  (let ((mono-spaced-font "Maple Mono")
    ...
    (set-face-attribute 'fixed-pitch nil :family mono-spaced-font :height 1.5)
    (set-face-attribute 'variable-pitch nil :family proportionately-spaced-font :height 1.5))
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (setq org-agenda-files (list (expand-file-name "~/Documents/notes/agenda/")))
...
#+end_src
* rss
#+begin_src emacs-lisp :var feed-script=(concat (file-name-parent-directory buffer-file-name) "private/feed.el")
  (use-package elfeed
    :ensure t
    :config
    (load feed-script)
